...
class GNNEncoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, edge_dim, dropout: float = 0.0):
        super().__init__()
...
class RNNEncoder(nn.Module):
    def __init__(self, hidden_channels, num_layers, dropout: float = 0.0):
        super().__init__()
        self.rnn = nn.LSTM(hidden_channels, hidden_channels, num_layers=num_layers,  batch_first=True, dropout=dropout)
...
class LitFullModel(Try1):
    ...
    def __init__(
        ...
        super().__init__(in_channels, hidden_channels, dropout_rate, out_classes, rnn_window_size, 
                         rnn_num_layers, binary_threshold, negative_edge_sampling_min, pred_alpha, link_pred_only, model_name)
        
        self.gnn = GNNEncoder(in_channels, hidden_channels, edge_dim, dropout_rate)
