...
class LANLHeteroGraphLoader(LANLRecordFetcher):
    def __init__(
        self,
        es: Elasticsearch,
        indexes: list[str] | str,
    ...
    def create_graph(self, records, curr_time):
        ...
        data = Data()
        data.x = torch.zeros((len(nodemap), 1), dtype=torch.float32)
        data.edge_index = torch.empty((2, num_records), dtype=torch.long)
        data.edge_attr = torch.empty((num_records, 4), dtype=torch.long)
        data.y = torch.zeros((num_records, 1), dtype=torch.float32)  # Default to 0
        
        


        data.time = torch.empty(num_records, dtype=torch.float32)
